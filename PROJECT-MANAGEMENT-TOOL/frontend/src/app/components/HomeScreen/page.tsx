...
export default function HomeScreen() {
    ...
    const fetchStats = async () => {
      try {
        const [projRes, userRes] = await Promise.all([
          fetch('http://localhost:8080/api/projects/stats/count'),
          fetch('http://localhost:8080/api/users/stats/count'),
        ]);
  ...
  const fetchUsers = async (page: number) => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:8080/api/users?page=${page}&limit=20&search=${encodeURIComponent(search)}`);
