...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      try {
        ...
        const userRes = await fetch('http://localhost:8080/api/users?page=1&limit=20');
        ...
        await fetch(`http://localhost:8080/api/send-task-reminders`, {
          method: 'POST',
    ...
    const loadMoreUsers = async () => {
      ...
      const res = await fetch(`http://localhost:8080/api/users?page=${nextPage}&limit=20`);
  ...
  const sendEmailNotification = async (task: any, projectId: string) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8080/api/send-task-reminders', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/projects/${id}/tasks/${editingTask._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleStatusChange = async (taskId: string, newStatus: Task['status']) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8080/api/projects/${id}/tasks/${taskId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRemovePerson = async (personId: string) => {
    try {
      const res = await fetch(`http://localhost:8080/api/projects/${id}/people/${personId}`, {
        method: 'DELETE',
      });
  ...
  const handleAddPersonToProject = async (user: User) => {
    try {
      ...
      const res = await fetch(`http://localhost:8080/api/projects/${id}/people`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
